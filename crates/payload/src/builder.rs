...
use crate::error::FraxtalPayloadBuilderError;

/// Optimism's payload builder
#[derive(Debug, Clone)]
pub struct FraxtalPayloadBuilder<EvmConfig, Txs = ()> {
...
impl<EvmConfig> FraxtalPayloadBuilder<EvmConfig> {
    /// `FraxtalPayloadBuilder` constructor.
    ///
...
impl<EvmConfig, Txs> FraxtalPayloadBuilder<EvmConfig, Txs> {
    ...
    pub fn with_transactions<T: OpPayloadTransactions>(
        self,
        best_transactions: T,
    ) -> FraxtalPayloadBuilder<EvmConfig, T> {
        let Self {
...
impl<EvmConfig, T> FraxtalPayloadBuilder<EvmConfig, T>
where
    EvmConfig: ConfigureEvm<Header = Header, Transaction = OpTransactionSigned>,
{
        ...
        Txs: PayloadTransactions<Transaction = OpTransactionSigned>,
    {
        ...
        let ctx = FraxtalPayloadBuilderCtx {
...
/// Implementation of the [`PayloadBuilder`] trait for [`FraxtalPayloadBuilder`].
impl<Pool, Client, EvmConfig, Txs> PayloadBuilder<Pool, Client>
    for FraxtalPayloadBuilder<EvmConfig, Txs>
where
    Client: StateProviderFactory + ChainSpecProvider<ChainSpec = OpChainSpec>,
    ...
    Txs: PayloadTransactions<Transaction = OpTransactionSigned>,
{
    ...
    pub fn execute<EvmConfig, DB>(
        self,
        state: &mut State<DB>,
        ctx: &FraxtalPayloadBuilderCtx<EvmConfig>,
        ...
        DB: Database<Error = ProviderError>,
    {
        ...
        // 2-bis. ensure frxUSD is force deployed
        ctx.ensure_frxusd(state)?;
...
#[derive(Debug)]
pub struct FraxtalPayloadBuilderCtx<EvmConfig> {
...
impl<EvmConfig> FraxtalPayloadBuilderCtx<EvmConfig> {
    ...
    pub fn ensure_frxusd<DB>(&self, db: &mut State<DB>) -> Result<(), PayloadBuilderError>
    where
        DB: Database,
        DB::Error: Display,
    {
        fraxtal_evm::frxusd::ensure_frxusd(
            self.chain_spec.clone(),
            self.attributes().payload_attributes.timestamp,
            db,
        )
        .map_err(|err| {
            warn!(target: "payload_builder", %err, "missing create2 deployer, skipping block.");
            PayloadBuilderError::other(FraxtalPayloadBuilderError::ForceFrxUSDFail)
        })
    }
}
